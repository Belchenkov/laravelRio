            ...
            <div class="sticky hentry hentry-post blog-big group">
                ...
                <div class="thumbnail">
                    ...
                    <div class="image-wrap">
                        <img
                            src="{{asset(env('THEME'))}}/images/articles/{{ $article->img->max }}"
                            alt="$article->title"
                            title="$article->title"
                        />
                    </div>
                ...
                <div class="meta group">
                    ...
                    <p class="comments">
                        <span>
                            <a
                                href="{{ route('articlesCat', ['cat_alias' => $article->category->alias]) }}#respond"
                                title="Comment on Section shortcodes &amp; sticky posts!"
                            >
                                {{ count($article->comments) ? count($article->comments) : '0' }}
                                {{ \Lang::choice('ru.comments', $article->comments) }}
                            </a>
                        </span>
                    </p>
                </div>
                ...
                <div class="the-content group">
                    <p>{!! $article->desc !!}</p>
                    <a
                        href="{{route('articles.show', ['alias' => $article->alias])}}"
                        class="btn btn-beetle-bus-goes-jamba-juice-4 btn-more-link">
                        → {{\Lang::get('ru.read_more')}}
                    </a>
                </div>
                <div class="clear"></div>
            </div>
